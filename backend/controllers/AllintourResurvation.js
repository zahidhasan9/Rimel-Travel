const alltourReserve = require("../models/AllinOneTourReservation");

const bookTour = async (req, res) => {
  try {
    console.log("ava");
    const newRes = new alltourReserve(req.body);
    console.log(newRes);
...
const getAllReservations = async (req, res) => {
  try {
    const allReservations = await alltourReserve.find();
...
const deleteReservation = async (req, res) => {
  const { id } = req.params;
  try {
    const deleted = await alltourReserve.findByIdAndDelete(id);
    if (!deleted) {
...
const getUserReservations = async (req, res) => {
  try {
    ...
    const userReservations = await alltourReserve.find({ currentUser });
